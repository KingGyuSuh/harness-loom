...
3. **Reset procedure — performed directly by the orchestrator, all in the current response**:
   ...
   b. Move `.harness/cycle/state.md`, `.harness/cycle/events.md`, and `.harness/cycle/epics/` into that archive directory. `.harness/loom/` and `.harness/docs/` stay untouched.
...
- Skip the cycle-end doc-keeper step at halt. That step preserves the documentation invariant for `.harness/docs/` and `CLAUDE.md`/`AGENTS.md`; bypassing it lets terminal cycles drift away from the doc snapshot the next cycle's planner will read.
