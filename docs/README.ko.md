...
정본은 `.claude/`이고, `.codex/`와 `.gemini/`는 필요할 때 여기에 맞춰 파생합니다.
...
- **정본은 한 곳에서만.** 하네스는 `.claude/`에서 작성하고, `.codex/`와 `.gemini/`는 필요할 때만 파생합니다.
- **훅 기반 실행.** 오케스트레이터가 다음 디스패치를 `.harness/state.md`에 써 두면, 훅이 수동 정리 없이 다음 사이클을 이어갑니다.
...
│   │   └── harness-context/
│   └── settings.json
└── 프로젝트별 producer/reviewer pair
```

이후 `/harness-pair-dev`로 도메인별 pair를 추가하고, 필요하면 `/harness-sync`로 Codex나 Gemini용 트리를 파생하면 됩니다.
...
- **지원되는 어시스턴트 CLI 최소 1개**, 인증 완료 상태:
  - [Claude Code](https://code.claude.com/docs) — 주 대상 플랫폼이며, `.claude/`가 정본입니다.
  - [Codex CLI](https://developers.openai.com/codex/cli) — `/harness-sync --provider codex`로 파생합니다.
  - [Gemini CLI](https://geminicli.com/docs/) — `/harness-sync --provider gemini`로 파생합니다.
...
또는 Claude Code 세션 안에서 마켓플레이스를 통해 설치할 수 있습니다.
...
그다음 마켓플레이스에서 `Harness Loom` 항목을 열고 플러그인을 설치합니다.

### Gemini CLI

Gemini는 `.agents/plugins/marketplace.json`을 통해 같은 플러그인 트리를 사용합니다. 리포지토리를 마켓플레이스 소스로 추가한 뒤 `Harness Loom`을 설치하세요.
...
산출물은 `.harness/epics/EP-N--<slug>/{tasks,reviews}/` 아래에 쌓입니다. 런타임 상태는 `.harness/state.md`, 이벤트 로그는 `.harness/events.md`에 남습니다.
...
- **EPIC / Task** — EPIC은 플래너가 정의한 성과 단위이고, Task는 그 EPIC 안에서 한 번 수행되는 producer-reviewer 라운드입니다. 관련 산출물은 `.harness/epics/EP-N--<slug>/{tasks,reviews}/`에 기록됩니다.
- **Orchestrator vs Planner** — **orchestrator**는 `.harness/state.md`를 소유하고 응답당 정확히 하나의 pair를 디스패치합니다. **planner**는 orchestrator의 루프 안에서 목표를 EPIC과 roster로 분해합니다.
...
| 플랫폼 | 모델 | 훅 이벤트 | 비고 |
|----------|-------|------------|-------|
| Claude | `inherit` | `Stop` | `.claude/settings.json`이 `.harness/hook.sh`를 트리거합니다. |
